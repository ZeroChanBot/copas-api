...
var creatorList = ['@zerogans','@rakugans','@zerobot','@zeroganteng'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6287834993722, key nya gratis kok gan, kalo kaliam subscribe channel ZEROBOT'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'zerogans') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'zerogans') return res.json(loghandler.invalidKey)
